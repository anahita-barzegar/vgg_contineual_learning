...
class AutoEncoder(ContinualLearner):
    ...
    def get_name(self):
        ...
        fcE_label = "{}_".format(self.fcE.name) if self.fc_layers > 1 else "{}{}_".format(
            "h" if self.depth > 0 else "i",
            self.conv_out_units)
    ...
    def encode(self, x, not_hidden=False):
        ...
        image_features = self.flatten(enc_b)
        # Forward-pass through fc-layers
        hE = self.fcE(image_features)
        # Get parameters for reparametrization
        (z_mean, z_logvar) = self.toZ(hE)

        return z_mean, z_logvar, hE, hidden_x, quant_t, quant_b, diff_t + diff_b, id_t, id_b
    ...
    def forward(self, x, gate_input=None, full=False, reparameterize=True, **kwargs):
        ...
        if full:
            ...
            return self.classify(x,
                                 reparameterize=reparameterize)  # -> if [full]=False, only forward pass for prediction
        ...
        if x_ is not None:
            ...
            for replay_id in range(n_replays):
                ...
                if (type(x_) == list) or (self.mask_dict is not None) or (TaskIL and self.dg_gates):
                    ...
                    if self.dg_gates and self.dg_type == "class":
                        if y_ is not None and y_[replay_id] is not None:
                            ...
                            if y_predicted.size(1) < self.classes:
                                ...
                                zeros_to_add_after = torch.zeros(n_batch,
                                                                 self.classes - (replay_id + 1) * y_predicted.size(1))
                    ...
                    # -run full model
                    gate_input = (
                        tasks_[replay_id] if self.dg_type == "task" else y_predicted) if self.dg_gates else None
...
class VQVAE(nn.Module):
    ...
    def label_map(self, labels, code_t, code_b, quant_t, quant_b):
        labeled_code_t = pd.DataFrame()
        labeled_code_b = pd.DataFrame()
        labeled_quant_t = pd.DataFrame()
        labeled_quant_b = pd.DataFrame()
        for i in range(len(labels)):
            labeled_code_t[i]['code_t'] = code_t[i]
            labeled_code_t[i]['category'] = labels[i]
            labeled_code_b[i]['code_b'] = code_b[i]
            labeled_code_b[i]['category'] = labels[i]
            labeled_quant_t[i]['quant_t'] = quant_t[i]
            labeled_quant_t[i]['category'] = labels[i]
            labeled_quant_b[i]['quant_b'] = quant_b[i]
            labeled_quant_b[i]['category'] = labels[i]

        return labeled_code_t, labeled_code_b, labeled_quant_t, labeled_quant_b

    def sample(self, num_samples, y):
        device = 'cpu'
        with torch.no_grad():
            # Generate random noise
            input = torch.randn(num_samples, 128).to(device)
            quant_t, quant_b, diff, _, _ = self.encode(input)
            dec = self.decode(quant_t, quant_b)
