...
class conv_layer_split(nn.Module):
    '''Convolutional layer outputting [mean] and [logvar] for each unit.'''

    def __init__(self, in_planes, out_planes, nl_mean=nn.ReLU(), nl_logvar=nn.Hardtanh(),
                 kernel_size=3, stride=1, padding=1, drop=0, batch_norm=False, bias=True, gated=False):
        super().__init__()
        self.mean = conv_layer(in_planes, out_planes, kernel_size=kernel_size, stride=stride, padding=padding,
                               drop=drop, batch_norm=batch_norm, bias=bias, gated=gated, nl=nl_mean)
        self.logvar = conv_layer(in_planes, out_planes, kernel_size=kernel_size, stride=stride, padding=padding,
                                 drop=drop, batch_norm=batch_norm, bias=False, gated=gated, nl=nl_logvar)
